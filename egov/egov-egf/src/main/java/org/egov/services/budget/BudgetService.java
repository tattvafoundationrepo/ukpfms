...
public class BudgetService extends PersistenceService<Budget, Long> {
	...
	/**
	 * return the department of any one budget detail will work for only leaf budget
	 * not for non leaf budgets
	 * 
	 * @param budget
	 * @return Department
	...
	public Department getDepartmentForBudget(final Budget budget) throws ApplicationRuntimeException {
		...
		String dept = null;
		final List<BudgetDetail> detailList = ((PersistenceService) this)
				.findAllBy("from  BudgetDetail budgetDetail where budgetDetail.budget=?", budget);
...
//            dept = empAssignment.getDepartment();
	...
	public boolean hasReForYear(final Long financialYear) {
		return checkForRe("from  Budget where financialYear.id=? and isbere='RE' and isActiveBudget=true",
				financialYear);
	}
	...
	public boolean hasApprovedBeForYear(final Long financialYear) {
		return checkForRe(new StringBuilder(
				...
				.append(" and isPrimaryBudget=true and status.code='Approved'").toString(), financialYear);
	}
	 ...
	 *
	 * @param financialYear
	 * @return boolean Finds out whether RE is created and Approved for the given
	 *         date
	 */
	...
	/**
	 * @param budgetsList
	 * @return The Base Budget Item is set from action class for which the details
	 *         are moved Budget b is used to avoid recursive class
	 */
	...
	public boolean canForwardParent(final Position position, final Budget b) {
		...
		final List<Budget> totalCountList = findAllBy("from Budget where parent=? and isActiveBudget=?",
				treeBudget.getParent(), true);
		// can forward the budget even though one of the child is not in the state of
		// forwarding if isActiveBudget is set as false
		final List<Budget> budgetList = findAllBy(
				"from Budget where state.owner=? and parent.id=? and isActiveBudget=?", position,
	...
	public List<CChartOfAccounts> getAccountCodeForBudgetHead(final List<BudgetGroup> bgroupList)
			throws ValidationException {

		if (bgroupList == null)
			throw new ValidationException(
					Arrays.asList(new ValidationError("BudgetGroup List is Null", "budgetgroup.list.is.null")));
		if (bgroupList.size() == 0)
			throw new ValidationException(
					Arrays.asList(new ValidationError("BudgetGroup List is Empty", "budgetgroup.list.is.empty")));
		final List<CChartOfAccounts> coaList = new ArrayList<CChartOfAccounts>();
		final Integer maxpossibleGlcodeLength = Integer
				.parseInt(EGovConfig.getProperty("egf_config.xml", "glcodeMaxLength", "", "AccountCode"));
		...
		for (final BudgetGroup bdgtgrp : bgroupList)
			if (((PersistenceService) this).find("from BudgetGroup where id = ? ", bdgtgrp.getId()) == null)
				throw new ValidationException(Arrays.asList(new ValidationError(
						"BudgetGroup with id:" + bdgtgrp.getId() + " and name:" + bdgtgrp.getName()
								+ " does not exist ",
						"BudgetGroup with id:" + bdgtgrp.getId() + " and name:" + bdgtgrp.getName()
								+ " does not exist ")));
			...
			if (bdgtgrp.getMajorCode() == null) {
				...
				if (maxCode != null && minCode != null) {
					...
					if (maxpossibleGlcodeLength == maxGlcodeStr.length()) {
						...
						glcodeFrom = minGlcodeStr
								+ multipleZeros.substring(0, maxpossibleGlcodeLength - minGlcodeStr.length());
						glcodeTo = maxGlcodeStr
								+ multipleNines.substring(0, maxpossibleGlcodeLength - maxGlcodeStr.length());
					}
	...
	public List getFYForNonApprovedBudgets() {
		return findAllBy(new StringBuilder(
				"select distinct b.financialYear from Budget b where b.status.code=!'Approved'")
				.append(" and isActiveBudget=true and isPrimaryBudget=true order by b.financialYear.finYearRange desc")
				.toString());
	}

	public Budget getBudget(String budgetHead, String deptCode, String budgetType, String fyear) {
		String budgetName;
		if (budgetHead.substring(0, 1).equalsIgnoreCase("1") || budgetHead.substring(0, 1).equalsIgnoreCase("2"))
	...
	/*
	 * public List<Budget> getBudgetsForUploadReport() { return findAllBy( new
	 * StringBuilder("select distinct b from Budget b where b.name like '%RE%' and b.materializedPath"
	...
	public List<Budget> getBudgetsForUploadReport() {
		...
		List<String> materializedPathList = getBudgetDetailForUploadReport();

		List<String> numberListBeforeDot = new ArrayList<String>();

		int dotIndex = 0;
		String result = null;

		for (String budget : materializedPathList) {

			dotIndex = budget.indexOf('.');
			result = dotIndex > 0 ? budget.substring(0, dotIndex) : budget;
			if (!numberListBeforeDot.contains(result))
				numberListBeforeDot.add(result);
		}

		String materializedPathForQuery = numberListBeforeDot.stream()
                .map(s -> "\'" + s + "\'")
                .collect(Collectors.joining(","));

		String query = new StringBuilder(
				"select distinct b from Budget b where b.name like '%RE%' and b.materializedPath").append(" in (")
				.append(materializedPathForQuery).append(")").toString();

		List<Budget> finalBudget = findAllBy(query);

		return finalBudget;
	}

	private List<String> getBudgetDetailForUploadReport() {

		final StringBuilder query = new StringBuilder();
		query.append("select distinct bd.materializedPath from BudgetDetail bd where bd.status.code = 'Created'");
