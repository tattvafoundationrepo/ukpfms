...
@ParentPackage("egov")
@Results({ @Result(name = "detailed", location = "chartOfAccounts-detailed.jsp"),
		@Result(name = "secondaryCoa", location = "secondaryChartOfAccounts-addSecNew.jsp"),		
		...
		@Result(name = "view-secondary-coa", location = "chartOfAccounts-viewSecondaryCoa.jsp"),
		@Result(name = Constants.VIEW_MODIFY_COA, location = "chartOfAccounts-viewModifyCoa.jsp"),
		@Result(name = "view-modified-secondary-coa", location = "chartOfAccounts-viewModifySecondaryCoa.jsp"),
		@Result(name = "new", location = "chartOfAccounts-new.jsp"),
		@Result(name = "detailed-editCode", location = "chartOfAccounts-editCode.jsp"),
		@Result(name = "secondary-editCode", location = "chartOfAccounts-editSecondaryCode.jsp"),
		@Result(name = "edit-secondary", location = "chartOfAccounts-edit-secondary.jsp"),
		@Result(name = "detailed-viewCode", location = "chartOfAccounts-viewCode.jsp"),
		@Result(name = "secondary-viewCode", location = "chartOfAccounts-viewSecCode.jsp"),
...
public class ChartOfAccountsAction extends BaseFormAction {
	...
	List<CChartOfAccounts> allSecondaryChartOfAccounts;
	...
	public void prepare() {
		...
		allSecondaryChartOfAccounts = chartOfAccountsService.findAllBy("from CChartOfAccounts where classification=?",
				Long.valueOf(parentForDetailedCode));
		if (model != null)
	...
	void populateGlCodeLengths() {
		...
		getGlCodeLengths().put(5l, 3);// Number of digits after detailed code
	}
	...
	boolean hasReference(final Integer id, final String glCode) {
		...
		final Query query = persistenceService.getSession().createSQLQuery(queryString.toString())
				.setParameter("glCode", glCode, StringType.INSTANCE).setParameter("id", id, IntegerType.INSTANCE);
	...
	boolean validAddtition(final String glCode) {
		...
		strQuery.append(
				"select bd.billid from  eg_billdetails bd, chartofaccounts coa,  eg_billregistermis brm where coa.glcode =:glCode")
		...
		strQuery.append(
				" and sts.id not in (select id from egw_status where upper(moduletype) like '%BILL%' and upper(description) like '%CANCELLED%') ");
		final Query query = persistenceService.getSession().createSQLQuery(strQuery.toString()).setParameter("glCode",
				glCode, StringType.INSTANCE);
		final List list = query.list();
		if (!list.isEmpty())
			flag = false;
		if (flag) {
	...
	private Long findNextGlCode(final CChartOfAccounts parentCoa) {
		final String glcode = (String) persistenceService
				.find("select max(glcode) from CChartOfAccounts where parentId=?", parentCoa.getId());
	...
	void setClassification(final CChartOfAccounts parentCoa) {
		if (parentCoa.getClassification() == null)
			...
			model.setClassification(LONG_FOUR);
		else if (parentCoa.getClassification().equals(4l))
			model.setClassification(5l);
	}
	...
	void populateGlcode(final Long classification) {
		int a = glCodeLengths.get(classification);
	...
	@Action(value = "/masters/secondaryChartOfAccounts-viewSecCode")
	public String viewSecondaryCode() {
		allSecondaryChartOfAccounts = chartOfAccountsHibernateDAO.getSecondaryCodesList();
		return "secondary-viewCode";
	}
	
	@SkipValidation
	@Action(value = "/masters/chartOfAccounts-viewSecondarySearch")
	public String viewSecondarySearch() {
		if (glCode != null) {
			model = chartOfAccountsService.find("from CChartOfAccounts where classification=5 and glcode=?",
					glCode.split("-")[0]);
			if (model == null) {
	...
	/*
	 * Mike
	 * Secondary COA Modify
	 * 
	 * */
	
	@Action(value = "/masters/secondaryChartOfAccounts-editSecCode")
	public String editSecondaryCode() {
		allSecondaryChartOfAccounts = chartOfAccountsHibernateDAO.getSecondaryCodesList();
		return "secondary-editCode";
	}
	
	@SkipValidation
	@Action(value = "/masters/chartOfAccounts-modifySearchSecondary")
	public String modifySecondarySearch() {
	...
	@Action(value = "/masters/chartOfAccounts-update-secondary")
	public String updateSecondary() {
		if (StringUtils.isEmpty(model.getName())) {
			addActionError("Please Enter the Name");
			populateAccountDetailTypeList();
			populateCoaRequiredFields();
		...
		clearCache();
		coaId = model.getId();
		return "view-modified-secondary-coa";
	}
	...
	@Action(value = "/masters/secondaryChartOfAccounts-addSecNew")
	public String addNewSecondary() {
		populateCodeLengthSecondary();
		allChartOfAccounts = chartOfAccountsHibernateDAO.getDetailedCodesList();
		model = new CChartOfAccounts();
		return "secondaryCoa";
	}

	void populateCodeLengthSecondary() {
...
//		majorCodeLength = 3;
//		minorCodeLength = 5;
//		subMinorCodeLength = 5;
//		detailedCodeLength = 10;
	...
	@Action(value = "/masters/chartOfAccounts-createSecondary")
	public String createSecondary() {
		if (glCode != null) {
			final CChartOfAccounts parent = chartOfAccountsService.find("from CChartOfAccounts where glcode=?",
					glCode.split("-")[0]);
			if (parent == null) {
			...
			if (StringUtils.isEmpty(generatedGlcode) || StringUtils.isEmpty(newGlcode) || newGlcode.length() != 3 || generatedGlcode.length() != 7) {
				addActionMessage(getText("chartOfAccount.invalid.glcode"));
				return "secondaryCoa";
			}
			if (StringUtils.isEmpty(model.getName())) {
			...
			model.setType(parent.getType());
			setClassification(parent);
			String glCodeNew = generatedGlcode.concat(newGlcode);
			model.setGlcode(glCodeNew);
			model.setMajorCode(model.getGlcode().substring(0, majorCodeLength));
			setPurposeOnCoa();
			model.setIsActiveForPosting(activeForPosting);
			populateAccountDetailType();			
			model = chartOfAccountsService.persist(model);
			saveCoaDetails(model);
			addActionMessage(getText("chartOfAccount.secondary.saved"));
		} else
			addActionMessage(getText("chartOfAccount.no.data"));
		clearCache();
		// return Constants.VIEW_COA;
	...
	@Action(value = "/masters/chartOfAccounts-ajaxNextSecondaryGlCode")
	public String ajaxNextSecondaryGlCode() {
		final String parentGlcode = parameters.get("parentGlcode")[0];
		if (parentGlcode != null || !StringUtils.isBlank(parentGlcode)) {
			final CChartOfAccounts coa = chartOfAccountsService.find("from CChartOfAccounts where glcode=?",
					parentGlcode);
			...
			if (glCode == null) {
				...
				newGlcode = String.valueOf(glCode + 1);
				extractDetailCodeForSecondary();
			}
		}
		return "generated-glcode";
	}

	void extractDetailCodeForSecondary() {

		newGlcode = newGlcode.substring(7, newGlcode.length());
	}
	...
	public List<CChartOfAccounts> getAllSecondaryChartOfAccounts() {
		return allSecondaryChartOfAccounts;
	}
